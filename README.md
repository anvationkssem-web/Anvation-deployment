...
in the Prisma/Vercel SQL console is optional but useful for confirming the
database connection. To migrate an existing local `server-data.json` once,
set the same connection variable locally and run. This migrates registrations
and the existing admin records; all subsequent portal mutations are saved
automatically:
...
In Vercel, open **Project Settings -> Environment Variables**, make sure the
Postgres URL is enabled for **Production**, and redeploy. Do not put the
connection URL in frontend `VITE_*` variables or commit it to the repository.
