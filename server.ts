...
<div style="background:#f1f5f9;padding:14px 26px;font-size:12px;color:#64748b;">For any help, write to anvation2026@kssem.edu.in · Sent ${new Date().toLocaleString()}</div>
  ...
  if (productionStoreEnabled) {
    try {
      ...
      if (Array.isArray(savedWebsiteState.sponsors)) sponsors = savedWebsiteState.sponsors as Sponsor[];
      if (Array.isArray(savedWebsiteState.milestoneReports)) milestoneReports = savedWebsiteState.milestoneReports as MilestoneReport[];
      if (Array.isArray(savedWebsiteState.mentorBookings)) mentorBookings = savedWebsiteState.mentorBookings as MentorBooking[];
      if (Array.isArray(savedWebsiteState.tickets)) tickets = savedWebsiteState.tickets as SupportTicket[];
      if (savedWebsiteState.cmsConfig && typeof savedWebsiteState.cmsConfig === "object") cmsConfig = savedWebsiteState.cmsConfig as WebsiteCMSConfig;
      if (savedWebsiteState.liveBroadcastAlert && typeof savedWebsiteState.liveBroadcastAlert === "object") liveBroadcastAlert = savedWebsiteState.liveBroadcastAlert as typeof liveBroadcastAlert;
      if (Array.isArray(savedWebsiteState.adminUsers)) adminUsers = savedWebsiteState.adminUsers as AdminUser[];
      if (Array.isArray(savedWebsiteState.checkpoints)) checkpoints = savedWebsiteState.checkpoints as Checkpoint[];
      if (Array.isArray(savedWebsiteState.auditLogs)) auditLogs = savedWebsiteState.auditLogs as AuditLog[];
      if (Array.isArray(savedWebsiteState.rulebooks)) rulebooks = savedWebsiteState.rulebooks as RulebookVersion[];
      if (Array.isArray(savedWebsiteState.emailCampaigns)) emailCampaigns = savedWebsiteState.emailCampaigns as EmailCampaign[];
      if (Array.isArray(savedWebsiteState.roomAllocations)) roomAllocations = savedWebsiteState.roomAllocations as RoomAllocation[];
      if (Array.isArray(savedWebsiteState.judgingRounds)) judgingRounds = savedWebsiteState.judgingRounds as JudgingRound[];
